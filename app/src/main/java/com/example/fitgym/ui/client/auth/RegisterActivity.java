...
public class RegisterActivity extends AppCompatActivity {
    ...
    private void syncOfflineClients() {
        ...
        for (Client c : offlineClients) {
            ...
            // Ignore les clients déjà synchronisés
            ...
            if (email.isEmpty() || !android.util.Patterns.EMAIL_ADDRESS.matcher(email).matches()) {
                Log.w("OfflineSync", "Email invalide ou vide: " + email);
                continue; // Skip ce client
            }

            // Si Email/Password, vérifie mot de passe
            if (!c.isGoogleSignIn() && pass.isEmpty()) {
                Log.w("OfflineSync", "Mot de passe vide pour Email/Password: " + email);
                continue; // Skip ce client
            }

            if (c.isGoogleSignIn()) {
                // Google Sign-In → vérifier si déjà sur Firebase
                firebaseHelper.getClientById(c.getId(), existing -> {
                    if (existing == null) firebaseHelper.ajouterClient(c, success -> {});
                    c.setSynced(true);
                    dao.modifierClient(c);
                    Log.i("OfflineSync", "Google account synced: " + email);
                });
            } else {
                // Email/Password → vérifier si déjà sur Firebase
                mAuth.fetchSignInMethodsForEmail(email).addOnCompleteListener(task -> {
                    if (!task.isSuccessful()) {
                        Log.e("OfflineSync", "Erreur fetchSignInMethods: " + task.getException());
                        return;
                    }

                    List<String> providers = task.getResult().getSignInMethods();
                    if (providers != null && !providers.isEmpty()) {
                        // Compte déjà existant → juste sync local
                        c.setSynced(true);
                        dao.modifierClient(c);
                        Log.i("OfflineSync", "Compte déjà existant, juste sync: " + email);
                    } else {
                        // Création Firebase
                        mAuth.createUserWithEmailAndPassword(email, pass)
                                .addOnCompleteListener(createTask -> {
                                    if (createTask.isSuccessful()) {
                                        ...
                                        Log.i("OfflineSync", "Compte créé sur Firebase: " + email);
                                    } else {
                                        Log.e("OfflineSync", "Erreur création Firebase: " + createTask.getException());
                                    }
                                });
                    }
                });
